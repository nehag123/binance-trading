	...
	public function __construct() {
	   ...
	   // $google_verified = $this->session->userdata('verified');
		...
		//$this->binApi = new Twilio\Rest\Client($sid, $token);
 		...
 		//$account   =  'hanaan' - CHANGED TO CRUN ON DIFFERENT ACCOUNT
 		//$account   = 'ubc';

// 		$data_HANAANTEST = array();
// 		$data_HANAANTEST = $this->session->userdata();
// 		$account = $data_HANAANTEST['account'];
		//$post = $this->input->post();

		//$post = $this->input->post();
		//$switchToAccount = $post['switch_account_code'];
        ...
        if($trade_account == 0)
        {
			$this->account= 'ubc';
				...
				$this->account= getTradingAccount($trade_account);
				
				}
		
	    //$account = isset($switch_account)?$switch_account:'ubc';//'hanaan';//
	    if($switch_account)
	    {
...
//		$this->api = createBinanceApiObject($account = 'hanaan'); 
		$this->api = createBinanceApiObject($account); 

		$selected = $this->uri->segment(3);
		$this->selected_coin = isset($selected)?$selected:'' ;
		
## USING TEMP BALANCES FUNCTION	
		$api = createBinanceApiObject($account);
		$balances_from_api = $api->balances(true);
		$this->balances = $balances_from_api;
## USING OLD GETACCOUNTBALANCES FUNCTION	
...
##
	...
	public function index( $data = array()) {
		...
		$jsonString = '';
		$subhead    = '';
//	$account = 'hanaan'; // 'hanaan' - CHANGED TO CRUN ON DIFFERENT ACCOUNT
			//$account = $this->account; // 'hanaan' - CHANGED TO CRUN ON DIFFERENT ACCOUNT
		if($switch_account)
	    {
			$account= $switch_account;
		 }else{
		...
		$data['completedTradesArray'] = getCompletedTradesList($account);//array('NYC','BOS');  

		$data['stablecoinPricesFullArray'] = aggregateStablecoinData($account);//array('NYC','BOS');
 ...
 ##### ADDED BY HANAAN
	...
	public function selectCoin () {
	  ...
	  if($switch_account)
		...
		## PREVIOUS USED - UPDATED BY HANAAN FEB 11
		$api = createBinanceApiObject($account);
		$balances_from_api = $api->balances(true);
		...
		$thisAccountArray = Array($account=>Array('account'=> $data['account'] , 'balances'=>$balances_from_api, 'balace_total'=>$data['balanceTotal'],'balace_total_bitcoin'=>$data['balanceBitcoinTotal'],'balace_total_altcoin'=>$data['balanceAltcoinTotal'],'balace_total_cash'=>$data['balanceCashTotal']));
		...
		$data['account'] = $account;
		$data['activeTradesArray'] =   getActiveTradesList($account);
		$data['completedTradesArray'] = getCompletedTradesList($account);  

		$data['stablecoinPricesFullArray'] = aggregateStablecoinData($account);
	...
	public function binance_execute_trade()
	{
		...
		$btc_trade_ubc2=isset($_POST['btc_trade_hanaan'])?$post['btc_trade_hanaan']:'';
		$coin_trade_name2= isset($_POST['coin_trade_hanaan'])?$post['coin_trade_hanaan']:'';
		
		## ADDED TO COMPENSATE FOR PREVIOUS KB HARDCODING UBC AS ACCOUNT
		if ($account == 'hanaan'){
			$coin_trade_name = $coin_trade_name2;
		}
        ...
        $ts = date("Y-m-d G:i:s",time());
  		...
  		echo '<h2>main action: '.$main_action.'</h2>';
       
  		//echo '<h2>TEST 1: '.$_POST['coin_trade_hanaan'].'</h2>';
  		//echo '<h2>TEST 2: '.$_POST['btc_trade_hanaan'].'</h2>';
       ...
       if ($main_action == 'Execute Buy Tradeeee'){
			...
			$symbol = $_POST['trade_coin_symbol'];
       		$useStandbystatusAndDoNotBuyCoin = 1;
       
		   	if ($useStandbystatusAndDoNotBuyCoin == 1){
		   		### THIS METHOD DOES NOT TRADE, IT SETS STATUS TO **STANDBY**
				$tradesArray = Array();
				$actionsArray = Array();
				$bidPrice = $bookPriceArray[$symbol.$sellCoinSymbol]['bid'];
				$coinInfoArray = Array('id'=>uniqid(),'action'=>'Initialize','account'=>$account,'symbol'=>$symbol,'status'=>'standby','is_rebuy'=>0,'is_armed'=>0, 'is_test'=>0,'qty'=>$accountCoinTradeAmount,'low_watched_price'=>$bidPrice,'initial_standby_price'=>$bidPrice, 'first_buy_price' => 0,'this_buy_price' => 0,'trades'=>$tradesArray,'actions'=>$actionsArray);
				$thisActionNotes = '';
				$thisAction = 'Initialize Trade';
				$coinInfoArray = addActionToActionsArray($coinInfoArray,$thisAction,$bookPriceArray,$symbol,$sellCoinSymbol,$thisActionNotes);
...
//ADD LATEST TRADE TO FILE ^ ^ ^
		...
		$data['btc_trade_ubc2']=$btc_trade_ubc2;
		$data['coin_trade_ubc2']=$coin_trade_name2;

		//$data['sellTradeResult']=$sellTradeResult;
